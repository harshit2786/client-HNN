...
function SinglePoem() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const resp = await getOneBlog("blogs", "Poem", poem);
    ...
    <div className="h-full flex items-center flex-col w-full">
      <div className="w-full px-8 border-[#BF7B67] flex justify-between items-center sticky border-b  h-20">
        <Breadcrumbs isDisabled>
          <BreadcrumbItem>
            <p className="text-[#e7946f] ">Poems</p>
          </BreadcrumbItem>
          <BreadcrumbItem>
      ...
      >
        <div className={`border-[#BF7B67] p-4 flex flex-col ${loader ? " justify-center" : ""} gap-2 bg-[#FAE9DD] shadow-lg border min-h-[200px]`}>
