...
function EditorOthers() {
    ...
    useEffect(()=> {
        const fetchNotes = async()=> {
            try{
                const resp = await getOneTypeBlog("blogs","Other");
                setOthers(resp.data);
  ...
  return (
    ...
    <Modal isOpen={isOpen} onClose={()=>setIsOpen(false)}>
        ...
        <ModalBody>
        <p className=' text-xs text-[#BF7B67] '> Type "DELETE" to permanently delete.</p>
    ...
    <div className='text-xl text-[#BF7B67]'>Others</div>
    
    <div className='bg-[#FAE9DD] h-[300px] border rounded-lg w-[80%] border-[#BF7B67] overflow-y-auto'>
      <Table removeWrapper>
        <TableHeader>
            <TableColumn><div className='item-center text-[#BF7B67] text-xs'>Serial no.</div></TableColumn>
        ...
        <TableBody emptyContent={"No items to display."}>
            {others?.map((item)=>(
                <TableRow  >
                    <TableCell><Chip className='text-[#BF7B67] bg-[#f7d2c7] font-light text-xs' size='sm' >Oth-{item?.id}</Chip></TableCell>
                    <TableCell>{item?.attributes?.Title}</TableCell>
                    <TableCell><Button onClick={()=> navigate(`/editor/others/${item?.attributes?.Path}`)} size='sm' isIconOnly variant='flat' className='bg-[#f7d2c7]'><CiEdit/></Button></TableCell>
