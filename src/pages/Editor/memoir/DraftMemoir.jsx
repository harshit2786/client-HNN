...
function DraftMemoir() {
    ...
    useEffect(()=> {
        const getData = async()=>{
            try{
                const resp = await getOneBlog("drafts","Memoir",draft);
  ...
  return (
    ...
    <div className={` ${isMobile ? "flex-col gap-2 items-center justify-center" : "justify-between"} w-full flex items-center `}>
      ...
      <div className={`w-full flex ${isMobile ? "flex-col justify-center items-center" : "justify-between h-[300px]"}  gap-2`}>
        <div className="flex flex-col items-center justify-center"><div className="text-lg text-[#BF7B67] font-light">Hi Kaddu Ji {`<3`}</div><Image style={{height:"250px"}} src="/memoir_1.gif" alt="gif"/></div>
