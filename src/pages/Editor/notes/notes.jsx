...
function EditorNotes() {
  ...
  return (
    ...
    <Modal isOpen={isOpen} onClose={()=>setIsOpen(false)}>
        ...
        <ModalBody>
        <p className=' text-xs text-[#BF7B67] '> Type "DELETE" to permanently delete this note.</p>
    ...
    <div className='bg-[#FAE9DD] h-[300px] border rounded-lg w-[80%] border-[#BF7B67] overflow-y-auto'>
      <Table removeWrapper>
        <TableHeader>
            <TableColumn><div className='item-center text-[#BF7B67] text-xs'>Note no.</div></TableColumn>
        ...
        <TableBody emptyContent={"No notes to display."}>
            {notes?.map((item)=>(
                <TableRow  >
                    <TableCell><Chip className='text-[#BF7B67] bg-[#f7d2c7] font-light text-xs' size='sm' >Note-{item?.id}</Chip></TableCell>
                    <TableCell>{item?.attributes?.Title}</TableCell>
                    <TableCell><Button onClick={()=> navigate(`/editor/notes/${item?.attributes?.Path}`)} size='sm' isIconOnly variant='flat' className='bg-[#f7d2c7]'><CiEdit/></Button></TableCell>
