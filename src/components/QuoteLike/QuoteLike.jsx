...
import { CreateData, getFilteredByTwoRelation } from '../../controllers/strapiController';
import Modal from './Modal';
function QuoteLike({quote,isOpen,setIsOpen}) {
    const userId = sessionStorage.getItem("userData")
    ? JSON.parse(sessionStorage.getItem("userData"))
    : null;
    const [userLike,setUserLike] = useState(false);
    const [numLikes,setNumLikes]= useState(quote?.attributes?.likes?.data?.length);
    ...
    useEffect(() => {
        const checkLike= async() => {
            try{
                const resp = await getFilteredByTwoRelation("likes","blog",quote.id,"user",userId.user.id);
